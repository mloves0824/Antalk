...
#include <iostream>
#include <unistd.h>
#include "phxqueue_handle_mgr.h"

namespace anrpc {
namespace queue {

comm::RetCode PhxQueueHandleMgr::Get(const comm::proto::GetRequest &req,
...
comm::RetCode PhxQueueHandleMgr::Add(const comm::proto::AddRequest &req,
    comm::proto::AddResponse &resp) {
    return comm::RetCode::RET_OK;
}

comm::RetCode PhxQueueHandleMgr::UncompressBuffer(const std::string &buffer, const int buffer_type,
    std::string &decoded_buffer) {
    decoded_buffer = buffer;
    return comm::RetCode::RET_OK;
}

void PhxQueueHandleMgr::RestoreUserCookies(const comm::proto::Cookies &user_cookies) {
}

void PhxQueueHandleMgr::CompressBuffer(const std::string &buffer, std::string &compress_buffer,
    const int buffer_type) {
    compress_buffer = buffer;
}


comm::RetCode PhxQueueHandleMgr::GetAddrScale(const comm::proto::GetAddrScaleRequest &req,
...
comm::RetCode PhxQueueHandleMgr::GetQueueByAddrScale(const std::vector<consumer::Queue_t> &queues,
...
comm::RetCode PhxQueueHandleMgr::GetLockInfo(const comm::proto::GetLockInfoRequest &req,
    comm::proto::GetLockInfoResponse &resp) {
    return comm::RetCode::RET_ERR_NO_IMPL;
}

comm::RetCode PhxQueueHandleMgr::AcquireLock(const comm::proto::AcquireLockRequest &req,
    comm::proto::AcquireLockResponse &resp) {
    return comm::RetCode::RET_ERR_NO_IMPL;
}

void PhxQueueHandleMgr::BeforeLock(const comm::proto::ConsumerContext &cc) {
    QLInfo("BeforeLock cc sub_id %d store_id %d queue_id %d",
        cc.sub_id(), cc.store_id(), cc.queue_id());
}

void PhxQueueHandleMgr::AfterLock(const comm::proto::ConsumerContext &cc) {
